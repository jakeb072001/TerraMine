...
public class MagicMissileEntity extends AbstractArrow {
    ...
    public static final ResourceLocation MAGIC_MISSILE_PARTICLE_ID = Artifacts.id("magic_missile_particle");
    ...
    private void trinketCheck() { // replace with AttributeModifier later
        if (TrinketsHelper.isEquipped(Items.SORCERER_EMBLEM, (LivingEntity) this.getOwner()) && TrinketsHelper.isEquipped(Items.AVENGER_EMBLEM, (LivingEntity) this.getOwner())) {
            damageIncrease = 1.27f;
        } else if (TrinketsHelper.isEquipped(Items.SORCERER_EMBLEM, (LivingEntity) this.getOwner())) {
            damageIncrease = 1.15f;
        } else if (TrinketsHelper.isEquipped(Items.AVENGER_EMBLEM, (LivingEntity) this.getOwner())) {
    ...
    private void explode()
    {
        if(!this.level.isClientSide)
        {
            ...
            playSound(SoundEvents.FALLING_STAR_CRASH,0.25f, 1.7f);
            this.kill();
        }
    }
    ...
    private void spawnMagicEffects() {
        ...
        this.level.addParticle(Artifacts.BLUE_POOF, position().x(), position().y(), position().z(), random, -0.2D, random);
    }
    ...
    private void spawnRainbowEffects() {
        float random = (rand.nextFloat() - 0.5F) * 0.1F;
        this.level.addParticle(ParticleTypes.FLAME, position().x(), position().y(), position().z(), random, -0.2D, random);
        this.level.addParticle(Artifacts.GREEN_SPARK, position().x(), position().y(), position().z(), random, -0.2D, random);
        this.level.addParticle(Artifacts.BLUE_POOF, position().x(), position().y(), position().z(), random, -0.2D, random);
    }
