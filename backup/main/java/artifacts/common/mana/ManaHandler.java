...
import terracraft.common.init.Statistics;
...
public class ManaHandler implements PlayerComponent<Component>, AutoSyncedComponent {
    ...
    public void update() {
        Components.MANA_HANDLER.sync(provider);
        ...
        if ((currentMana == maxMana && manaRegenDelay <= 0)) {
            ...
            provider.level.playSound(null, provider.blockPosition(), SoundEvents.MANA_FULL, SoundSource.PLAYERS, 1f, 1f);
    ...
    public void addCurrentMana(int currentMana) {
        if (provider != null && !provider.isCreative()) {
            if (currentMana > 0) {
                ...
                provider.awardStat(Stats.CUSTOM.get(Statistics.MANA_USED), Math.max(currentMana, 0));
