...
public class RocketBootsItem extends TrinketArtifactItem {
	...
	public void curioTick(LivingEntity player, ItemStack stack) {
		...
		{
			if (timer >= 10) {
				((Player) player).getCooldowns().addCooldown(Items.ROCKET_BOOTS, 30);
			}
			if (timer > 0)
			{
				timer = 0;
		...
		if (TrinketsHelper.isEquipped(Items.ROCKET_BOOTS, player) && !TrinketsHelper.isEquipped(Items.SPECTRE_BOOTS, player) && !TrinketsHelper.isEquipped(Items.LIGHTNING_BOOTS, player) && !TrinketsHelper.isEquipped(Items.FROSTSPARK_BOOTS, player)
				&& !TrinketsHelper.isEquipped(Items.TERRASPARK_BOOTS, player) && settings != null && timer < rocketTime && player instanceof Player user && !user.getCooldowns().isOnCooldown(Items.ROCKET_BOOTS)) {
				if (settings.keyJump.isDown()) {
					...
					if (settings.keyJump.isDown()) {
						...
						if (soundTimer >= 3) {
							player.level.playSound((Player) player, player.blockPosition(), SoundEvents.ROCKET_BOOTS, SoundSource.PLAYERS, 0.5f, 1f);
							soundTimer = 0;
						}
