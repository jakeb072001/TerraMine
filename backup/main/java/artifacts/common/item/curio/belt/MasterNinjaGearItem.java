...
public class MasterNinjaGearItem extends TrinketArtifactItem {
	...
	protected void curioTick(LivingEntity livingEntity, ItemStack stack) {
		...
		if (mc.options.keyUp.isDown()) {
			if (mc.options.keyUp.isDown() && upPressed && upKeyUnpressed && !player.getCooldowns().isOnCooldown(Items.MASTER_NINJA_GEAR) && !player.isInWaterOrBubble()) {
				for (int i = 0; i < 20; ++i) {
				...
				player.getCooldowns().addCooldown(Items.MASTER_NINJA_GEAR, 20);
				upPressed = false;
				upKeyUnpressed = false;
			} else {
				upPressed = true;
		...
		if (mc.options.keyDown.isDown()) {
			if (mc.options.keyDown.isDown() && downPressed && downKeyUnpressed && !player.getCooldowns().isOnCooldown(Items.MASTER_NINJA_GEAR) && !player.isInWaterOrBubble()) {
				for (int i = 0; i < 20; ++i) {
					double d0 = mc.level.random.nextGaussian() * 0.02D;
					double d1 = mc.level.random.nextGaussian() * 0.02D;
					double d2 = mc.level.random.nextGaussian() * 0.02D;
		...
		if (mc.options.keyLeft.isDown()) {
			if (mc.options.keyLeft.isDown() && leftPressed && leftKeyUnpressed && !player.getCooldowns().isOnCooldown(Items.MASTER_NINJA_GEAR) && !player.isInWaterOrBubble()) {
				for (int i = 0; i < 20; ++i) {
					double d0 = mc.level.random.nextGaussian() * 0.02D;
					double d1 = mc.level.random.nextGaussian() * 0.02D;
					double d2 = mc.level.random.nextGaussian() * 0.02D;
		...
		if (mc.options.keyRight.isDown()) {
			if (mc.options.keyRight.isDown() && rightPressed && rightKeyUnpressed && !player.getCooldowns().isOnCooldown(Items.MASTER_NINJA_GEAR) && !player.isInWaterOrBubble()) {
				for (int i = 0; i < 20; ++i) {
					double d0 = mc.level.random.nextGaussian() * 0.02D;
					double d1 = mc.level.random.nextGaussian() * 0.02D;
					double d2 = mc.level.random.nextGaussian() * 0.02D;
