...
public class RocketBootsItem extends TrinketTerrariaItem {
	...
	public void curioTick(LivingEntity player, ItemStack stack) {
		...
		if (TrinketsHelper.isEquipped(ModItems.ROCKET_BOOTS, player) && !TrinketsHelper.isEquipped(ModItems.SPECTRE_BOOTS, player) && !TrinketsHelper.isEquipped(ModItems.LIGHTNING_BOOTS, player) && !TrinketsHelper.isEquipped(ModItems.FROSTSPARK_BOOTS, player)
				&& !TrinketsHelper.isEquipped(ModItems.TERRASPARK_BOOTS, player) && settings != null && timer < rocketTime && player instanceof Player user && !user.getCooldowns().isOnCooldown(ModItems.ROCKET_BOOTS)) {
				if (settings.keyJump.isDown()) {
					...
					if (settings.keyJump.isDown()) {
						...
						if (soundTimer >= 3) {
							player.level.playSound((Player) player, player.blockPosition(), ModSoundEvents.ROCKET_BOOTS, SoundSource.PLAYERS, 0.5f, 1f);
							soundTimer = 0;
						}
