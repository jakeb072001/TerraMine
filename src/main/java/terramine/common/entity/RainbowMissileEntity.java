...
public class RainbowMissileEntity extends MagicMissileHelper {
    ...
    public void createParticles() {
        ...
        this.level.addParticle(TerraMine.GREEN_SPARK, position().x(), position().y(), position().z(), random, -0.2D, random);
        this.level.addParticle(TerraMine.BLUE_POOF, position().x(), position().y(), position().z(), random, -0.2D, random);
    }
}
